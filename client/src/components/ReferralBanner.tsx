...
export default function ReferralBanner({ variant = "full", className = "" }: ReferralBannerProps) {
  if (variant === "compact") {
    return (
      <div
        className={`rounded-xl p-5 text-white ${className}`}
        style={{ background: 'linear-gradient(135deg, oklch(0.22 0.08 262), oklch(0.30 0.12 262))' }}
      >
        <p className="text-xs text-slate-300 mb-1 uppercase tracking-wider font-semibold">Exclusive Referral Code</p>
        <p className="font-mono font-black text-2xl text-emerald-400 tracking-widest mb-1">REFER200</p>
        <p className="text-slate-300 text-xs mb-4 leading-relaxed">
          Up to <strong className="text-white">£200 free cash</strong> when you open a Tide account
        </p>
        <a
          href={REFERRAL_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="cta-primary block w-full text-center px-4 py-2.5 rounded-lg text-sm font-semibold no-underline"
        >
          Claim £200 Free →
        </a>
      </div>
